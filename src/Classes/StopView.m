...
@property(nonatomic) UIBarButtonItem *segButton;
...
- (void)fetchDestinationsAsync:(id<TaskController>)taskController
    ...
    if (!self.backgroundRefresh &&
        [self.stopData getStopsAfterStopId:dep.stopId
                                     route:dep.route
                                 direction:dep.dir
                               description:dep.shortSign
        ...
        [taskController taskRunAsync:^(TaskState *taskState) {
          ...
          [self.stopData
              getStopsAfterStopId:dep.stopId
                            route:dep.route
                        direction:dep.dir
                      description:dep.shortSign
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    case kSectionRowRouteName: {
        ...
        PtrConstRouteInfo info =
            [TriMetInfo infoForRoute:self.stopData.routeId];
        [railCell populateCellWithStation:self.stopData.routeDescription
                                              .safeEscapeForMarkUp
    ...
    case kRowStop: {
        ...
        if (stop.timePoint) {
            ...
            /*
             [self newLabelWithPrimaryColor:[UIColor blueColor]
             selectedColor:[UIColor cyanColor] fontSize:14 bold:YES
        ...
        cell.accessibilityLabel =
            [NSString stringWithFormat:@"%@ %@", stop.desc,
                                       stop.timePoint ? @". Time point" : @""]
                .phonetic;
        break;
    }
    ...
    case kSectionRowTimePoint: {
        cell = [self tableView:tableView
            multiLineCellWithReuseIdentifier:@"tp"
                                        font:self.smallFont];
        cell.textLabel.attributedText =
            NSLocalizedString(
                @"#UBlue stops are #iTime Points#i - one of several stops on "
                @"each route that serves as a benchmark for whether a trip is "
                @"running on time.#D",
                @"information")
...
- (void)tableView:(UITableView *)tableView
    ...
    case kRowStop: {
        ...
        if (items != nil && indexPath.row >= items.count) {
            if (self.stopData.items == nil) {
                [self networkTips:self.stopData.htmlError
                     networkError:self.stopData.networkErrorMsg];
                [self clearSelection];
            }
...
- (NSString *)returnStopObjectActionText {
    if (self.stopIdStringCallback) {
        ...
        return NSLocalizedString(@"Show departure time at this stop",
                                 @"menu item");
    }
...
- (void)returnStopObject:(Stop *)stop progress:(id<TaskController>)progress {
    if (self.stopIdStringCallback) {
        /*
         ...
         {
         [self.navigationController popToViewController:[self.callback
         getController] animated:YES];
         }*/
    ...
    if (self.departure == nil) {
        ...
        departureViewController.displayName = stop.desc;
        [departureViewController fetchTimesForBlockAsync:progress
                                                   block:self.departure.block
                                                   start:self.departure.stopId
                                                  stopId:stop.stopId];
    }
}
...
- (void)toggleSort:(UISegmentedControl *)sender {
    ...
    {
        [self.stopData.items
            sortUsingSelector:@selector(compareUsingStopName:)];
        [self reloadData];
        break;
    }
    }
