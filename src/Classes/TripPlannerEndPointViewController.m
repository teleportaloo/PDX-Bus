//
//  TripPlannerEndPointViewController.m
//  PDX Bus
//
...
#define kStartTextDescPlaceHolder                                              \
    NSLocalizedString(@"<starting place or ID>", @"input placeholder")
#define kDestinationTextDescPlaceHolder                                        \
    NSLocalizedString(@"<destination place or ID>", @"input placeholder")
#define kTextGPSPlaceHolder                                                    \
    NSLocalizedString(@"<using current location (GPS)>", @"input placeholder")
...
@implementation TripPlannerEndPointViewController
...
- (void)hideKeyboard {
    if (_keyboardUp) {
        ...
        if (self.placeNameField.isFirstResponder &&
            self.placeNameField.canResignFirstResponder) {
...
- (UIPlaceHolderTextView *)createTextField_Rounded {
    CGRect frame =
        CGRectMake(0.0, 0.0, 80.0, [CellTextField editHeight] * kTextMultiple);
    UIPlaceHolderTextView *returnTextField =
        [[UIPlaceHolderTextView alloc] initWithFrame:frame];
    ...
    returnTextField.autocorrectionType =
        UITextAutocorrectionTypeYes; // no auto correction support
...
- (void)nextScreen {
    if (self.popBackTo) {
        [self.navigationController popToViewController:self.popBackTo
                                              animated:YES];
    } else {
...
- (void)gotPlace:(NSString *)place
    ...
    if (place.length != 0) {
        ...
        if (self.endPoint == nil ||
            ![place isEqualToString:[self endPoint].locationDesc]) {
            [self initEndPoint];
...
- (BOOL)cellShouldBeginEditing:(EditableTableViewCell *)cell {
    ...
    [self.tableView
        scrollToRowAtIndexPath:
            [self firstIndexPathOfSectionType:kTableSectionEnterDestination
                                      rowType:kTableEnterRowEnter]
...
- (void)selectFromRailMap {
    RailMapViewController *railMapView = [RailMapViewController viewController];
...
- (NSString *)tableView:(UITableView *)tableView
    ...
    case kTableSectionEnterDestination:
        ...
        if (self.from) {
            return NSLocalizedString(@"Choose starting address, or stop:",
                                     @"section header");
        } else {
            return NSLocalizedString(@"Choose destination, or stop:",
                                     @"section header");
        }
    ...
    case kTableSectionRowFaves:
        ...
        if (self.tripQuery.userFaves != nil &&
            self.tripQuery.userFaves.count > 0) {
...
- (bool)multipleStopsForFave:(NSInteger)index {
    UserParams *item = self.tripQuery.userFaves[index].userParams;
    NSString *location = item.valLocation;
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    case kTableEnterRowEnter: {
        if (self.editCell == nil) {
            self.editCell =
                [[CellTextView alloc] initWithStyle:UITableViewCellStyleDefault
            ...
            if ([self endPoint].useCurrentLocation) {
                ...
                if (self.from) {
                    ...
                    self.placeNameField.placeholder =
                        kDestinationTextDescPlaceHolder;
                }
            }
        }

        if ([self endPoint] != nil && [self endPoint].locationDesc != nil &&
            ![self endPoint].useCurrentLocation) {
            self.editCell.view.text = [self endPoint].locationDesc;
        } else {
            self.editCell.view.text = @"";
        }
    ...
    case kTableEnterRowBrowse: {
        UITableViewCell *cell = [self tableView:tableView
                        cellWithReuseIdentifier:kPlainFieldId];

        if (self.from) {
            cell.textLabel.text = NSLocalizedString(@"Browse for starting stop",
                                                    @"main menu item");
        } else {
            cell.textLabel.text = NSLocalizedString(
                @"Browse for destination stop", @"main menu item");
        }

        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        cell.systemIcon = kSFIconBrowse;
    ...
    case kTableEnterRowRailMap: {
        ...
        if (self.from) {
            cell.textLabel.text =
                NSLocalizedString(@"Select from rail maps", @"main menu item");
        } else {
            cell.textLabel.text =
                NSLocalizedString(@"Select from rail maps", @"main menu item");
        }
    ...
    case kTableEnterRowRailStations: {
        UITableViewCell *cell = [self tableView:tableView
                        cellWithReuseIdentifier:kPlainFieldId];
        cell.textLabel.text = NSLocalizedString(
            @"Search all rail stations (A-Z)", @"main menu item");
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        cell.namedIcon = kIconRailStations;
        cell.textLabel.font = self.basicFont;
    ...
    case kTableEnterRowContacts: {
        UITableViewCell *cell = [self tableView:tableView
                        cellWithReuseIdentifier:kPlainFieldId];
        cell.textLabel.text =
            NSLocalizedString(@"Address from contacts", @"main menu item");

        if (self.from) {
            [cell
                setAccessibilityLabel:
                    NSLocalizedString(@"Choose starting address from contacts",
                                      @"main menu item")];
        } else {
            [cell setAccessibilityLabel:
                      NSLocalizedString(
                          @"Choose destination address from contacts",
                          @"main menu item")];
        }

        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        cell.systemIcon = kSFIconContacts;
        cell.textLabel.font = self.basicFont;
        return cell;
    }
    ...
    case kTableSectionRowFaves: {
        ...
        // Set up the cell
        UserParams *item = self.tripQuery.userFaves[indexPath.row].userParams;
        // printf("item %p\)n", item);

        cell.textLabel.text = item.valChosenName;
        cell.textLabel.font = self.basicFont;
    ...
    case kTableSectionRowLocate: {
        ...
        if (self.from) {
            cell.textLabel.text = NSLocalizedString(
                @"Start from current location (GPS)", @"main menu item");
        } else {
            cell.textLabel.text = NSLocalizedString(
                @"Go to current location (GPS)", @"main menu item");
        }
...
- (void)tableView:(UITableView *)tableView
    ...
    case kTableEnterRowContacts: {
        ...
        if ([CNContactStore class]) {
            ...
            picker.displayedPropertyKeys = [[NSArray alloc]
                initWithObjects:CNContactPostalAddressesKey, nil];
            ;
            picker.predicateForEnablingContact =
                [NSPredicate predicateWithFormat:@"postalAddresses.@count > 0"];
            picker.predicateForSelectionOfContact = [NSPredicate
                predicateWithFormat:@"postalAddresses.@count == 1"];
    ...
    case kTableSectionRowFaves: {
        ...
        if (![self multipleStopsForFave:indexPath.row]) {
            ...
            [self gotPlace:location
                     setUiText:YES
                additionalInfo:item.valChosenName];
        } else {
            TripPlannerBookmarkViewController *bmView =
                [TripPlannerBookmarkViewController viewController];
            bmView.stopIdStringCallback = self;
            bmView.from = self.from;

            // bmView.displayName = item[kUserFavesOriginalName];
            [bmView fetchNamesForStopIdsAsync:self.backgroundTask
                                      stopIds:location];
        }
 ...
 * @discussion The picker will be dismissed automatically after a contact or
 * property is picked.
 */
- (void)contactPickerDidCancel:(CNContactPickerViewController *)picker {
}

- (void)getAddress:(CNPostalAddress *)postalAddress
 ...
 * @discussion These delegate methods will be invoked when the user selects a
 * single contact or property.
 */
