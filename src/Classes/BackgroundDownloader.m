...
@property (strong, atomic) NSMutableDictionary<NSString *, NSString *>  * absQueryFromOriginal;
@property (strong, atomic) NSMutableDictionary<NSString *, BackgroundDownloadState *> *stateForAbsQuery;
...
- (bool)startFetchInBackground:(TriMetXML *)xml query:(NSString *)query completion:(backgroundCompletionHandler)completionHander; {
    @synchronized (self.absQueryFromOriginal) {
        ...
        NSString *fullQuery = [xml fullAddressForQuery:query];
        
        NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:fullQuery]];
        
        NSURLSessionConfiguration *config = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:@"PDX Bus background"];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite {
    @synchronized (self.absQueryFromOriginal) {
        ...
        if (totalBytesExpectedToWrite  > 0) {
            ...
            state.progress = [NSString stringWithFormat:@"%.0f%%", percentDone *100.0];
        } else {
            NSString *result = nil;
            
            if (bytesWritten < 1024) {
                result = [NSString stringWithFormat:@"%d Bytes", (int)bytesWritten];
            } else if (bytesWritten < (1024 * 1024)) {
                result = [NSString stringWithFormat:@"%.2f K", (((float)(bytesWritten)) / 1024.0)];
            } else {
                result = [NSString stringWithFormat:@"%.2f MB", ((float)(bytesWritten) / (1024 * 1024))];
            }
