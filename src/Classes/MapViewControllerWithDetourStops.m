//
//  MapViewControllerWithDetourStops.m
//  PDX Bus
//
...
@implementation MapViewControllerWithDetourStops
...
- (void)modifyMapViewFrame:(CGRect *)frame {
    if (self.detours.count == 1) {
        ...
        NSString *textToFormat =
            [self.detours.firstObject markedUpDescriptionWithoutInfo:nil];

        if (self.annotations.count == 0) {
            NSArray<NSString *> *stopIdArray =
                self.detours.firstObject.extractStops;

            if (stopIdArray.count == 1) {
                textToFormat = [NSString
                    stringWithFormat:
                        NSLocalizedString(
                            @"#b#RNo location found for stop %@.#b#D\n%@",
                            @"error message"),
                        stopIdArray.firstObject, textToFormat];
            } else {
                textToFormat = [NSString
                    stringWithFormat:
                        NSLocalizedString(
                            @"#b#RNo locations found for stops %@.#b#D\n%@",
                            @"error message"),
        ...
        // CGRect textSize = [text
        // boundingRectWithSize:CGSizeMake(frame->size.width, CGFLOAT_MAX)
        // options:NSStringDrawingUsesLineFragmentOrigin |
        // NSStringDrawingUsesFontLeading context:nil];

        CGFloat textHeight = frame->size.height * (1 - mapRatio);
        // CGFloat textHeight = textSize.size.height > maxTextHeight ?
        // maxTextHeight : textSize.size.height;

        CGRect textViewFrame = CGRectMake(
            frame->origin.x, frame->origin.y + frame->size.height - textHeight,
            frame->size.width, textHeight);
        ...
        if (self.detourText == nil) {
            self.detourText =
                [[LinkResponsiveTextView alloc] initWithFrame:textViewFrame];
            self.detourText.textAlignment = NSTextAlignmentLeft;
            self.detourText.backgroundColor =
            ...
            self.detourText.accessibilityTraits =
        ...
        // Now redo the size
        CGSize newSize = [self.detourText
            sizeThatFits:CGSizeMake(frame->size.width, MAXFLOAT)];

        textHeight = newSize.height > textHeight
                         ? textHeight
                         : (newSize.height + kTextMargin);
...
- (void)fetchLocationsMaybeAsync:(id<TaskController>)taskController
    ...
    for (Detour *detour in self.detours) {
        if (detour.routes) {
            for (Route *route in detour.routes) {
                [routes addObject:route.routeId];
            }
        }
    ...
    if (stopIds.count == 0 && self.annotations.count > 0) {
        [self fetchRoutesAsync:taskController
                        routes:routes
                    directions:nil
               additionalTasks:0
                          task:nil];
        ...
        [self fetchRoutesAsync:taskController
                        routes:routes
                    directions:nil
               additionalTasks:batches.count
                          task:^(TaskState *taskState) {
                            [XMLDepartures clearCache];

                            [taskState taskSubtext:NSLocalizedString(
                                                       @"getting stops",
                                                       @"progress message")];

                            for (NSString *allLocs in batches) {
                                XMLMultipleDepartures *allDeps =
                                    [XMLMultipleDepartures
                                         xmlWithOptions:DepOptionsNoDetours |
                                                        DepOptionsOneMin
...
- (BOOL)textView:(UITextView *)textView
    ...
    return [self detourLink:URL.absoluteString
                     detour:self.detours.firstObject
                     source:textView];
}
