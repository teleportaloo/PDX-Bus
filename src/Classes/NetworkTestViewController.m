...
- (void)subTaskCheckDepartures:(TaskState *)taskState {
    [taskState taskSubtext:NSLocalizedString(@"checking departure server",
                                             @"progress message")];
...
- (void)subTaskCheckTripServer:(TaskState *)taskState {
    [taskState taskSubtext:NSLocalizedString(@"checking trip server",
                                             @"progress message")];
...
- (void)subTaskCheckNextBus:(TaskState *)taskState {
    [taskState taskSubtext:NSLocalizedString(@"checking NextBus server",
                                             @"progress message")];

    XMLStreetcarLocations *locations = [XMLStreetcarLocations
        sharedInstanceForRoute:[TriMetInfo streetcarRoutes].anyObject];
    locations.giveUp = 7;
...
- (void)subTaskCheckGeoLocator:(TaskState *)taskState {
    if ([ReverseGeoLocator supported]) {
        [taskState taskSubtext:NSLocalizedString(@"checking geocoder server",
                                                 @"progress message")];
...
- (void)subTaskCheckConnectivity:(TaskState *)taskState {
    [taskState taskSubtext:NSLocalizedString(@"checking connection",
                                             @"progress message")];
...
- (void)fetchNetworkStatusAsync:(id<TaskController>)taskController
              backgroundRefresh:(bool)backgroundRefresh {
    [taskController taskRunAsync:^(TaskState *taskState) {
      self.backgroundRefresh = backgroundRefresh;

      [taskState taskStartWithTotal:5
                              title:NSLocalizedString(@"checking network",
                                                      @"progress message")];
      ...
      if (!self.internetConnectionStatus) {
          [diagnosticString
              appendString:
                  NSLocalizedString(
                      @"The Internet is not available. Check you are not in "
                      @"Airplane mode, and not in the Robertson Tunnel.\n\nIf "
                      @"your device is capable, you could also try switching "
                      @"between WiFi, Edge and 3G.\n\nTouch here to start "
                      @"Safari to check your connection. ",
                      @"error message")];
      } else if (!self.trimetQueryStatus || !self.nextbusQueryStatus ||
                 !self.trimetTripStatus) {
          [diagnosticString
              appendString:
                  NSLocalizedString(
                      @"The Internet is available, but PDX Bus is not able to "
                      @"contact TriMet's or NextBus's servers. Touch here to "
                      @"check if www.trimet.org is working.",
                      @"error message")];
      } else {
          [diagnosticString
              appendString:NSLocalizedString(
                               @"The main network services are working at this "
                               @"time. If you are having problems, touch here "
                               @"to load www.trimet.org, then restart PDX Bus.",
                               @"error message")];
      }
...
/*
 ...
 want to perform customization that is not appropriate for viewDidLoad. if (self
...
- (UITableViewCell *)networkStatusCell {
    UITableViewCell *cell = [self tableView:self.tableView
                    cellWithReuseIdentifier:MakeCellId(networkStatusCell)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    case kSectionInternet:
        ...
        if (!self.internetConnectionStatus) {
            cell.textLabel.text = NSLocalizedString(
                @"Not able to access the Internet", @"network error");
            cell.textLabel.textColor = [UIColor redColor];
            [cell systemIcon:kSFIconNetworkBad tint:kSFIconNetworkBadTint];
            cell.textLabel.font = self.basicFont;
        } else {
            cell.textLabel.text = NSLocalizedString(
                @"Internet access is available", @"network error");
            [cell systemIcon:kSFIconNetworkOk tint:kSFIconNetworkOkTint];
    ...
    case kSectionTriMet:
        ...
        if (!self.trimetQueryStatus) {
            cell.textLabel.text = NSLocalizedString(
                @"Not able to access TriMet departure servers",
                @"network error");
            cell.textLabel.textColor = [UIColor redColor];
            [cell systemIcon:kSFIconNetworkBad tint:kSFIconNetworkBadTint];
            cell.textLabel.font = self.basicFont;
            ...
            cell.textLabel.text = NSLocalizedString(
                @"TriMet departure servers are available", @"network errror");
            cell.textLabel.textColor = [UIColor modeAwareText];
            [cell systemIcon:kSFIconNetworkOk tint:kSFIconNetworkOkTint];
            cell.textLabel.font = self.basicFont;
        }
    ...
    case kSectionTriMetTrip:
        ...
        if (!self.trimetTripStatus) {
            cell.textLabel.text = NSLocalizedString(
                @"Not able to access TriMet trip servers", @"network errror");
            cell.textLabel.textColor = [UIColor redColor];
            [cell systemIcon:kSFIconNetworkBad tint:kSFIconNetworkBadTint];
            cell.textLabel.font = self.basicFont;
        } else {
            cell.textLabel.text = NSLocalizedString(
                @"TriMet trip servers are available", @"network errror");
    ...
    case kSectionNextBus:
        ...
        if (!self.nextbusQueryStatus) {
            cell.textLabel.text = NSLocalizedString(
                @"Not able to access Umo IQ (Streetcar) servers",
                @"network errror");
            cell.textLabel.textColor = [UIColor redColor];
            [cell systemIcon:kSFIconNetworkBad tint:kSFIconNetworkBadTint];
            cell.textLabel.font = self.basicFont;
            ...
            cell.textLabel.text = NSLocalizedString(
                @"Umo IQ (Streetcar) servers are available", @"network errror");
            cell.textLabel.textColor = [UIColor modeAwareText];
            [cell systemIcon:kSFIconNetworkOk tint:kSFIconNetworkOkTint];
            cell.textLabel.font = self.basicFont;
        }
    ...
    case kSectionReverseGeoCode:
        ...
        if (self.reverseGeoCodeService == nil) {
            cell.textLabel.text = NSLocalizedString(
                @"No Reverse GeoCoding service is not supported.",
                @"network errror");
            cell.textLabel.textColor = [UIColor modeAwareText];
            [cell systemIcon:kSFIconNetworkOk tint:kSFIconNetworkOkTint];
            cell.textLabel.font = self.basicFont;
            ...
            cell.textLabel.text = NSLocalizedString(
                @"Not able to access Apple's Geocoding servers.",
                @"network errror");
            cell.textLabel.textColor = [UIColor redColor];
            [cell systemIcon:kSFIconNetworkBad tint:kSFIconNetworkBadTint];
            cell.textLabel.font = self.basicFont;
            ...
            cell.textLabel.text = NSLocalizedString(
                @"Apple's Geocoding servers are available.", @"network errror");
            cell.textLabel.textColor = [UIColor modeAwareText];
            [cell systemIcon:kSFIconNetworkOk tint:kSFIconNetworkOkTint];
            cell.textLabel.font = self.basicFont;
        }
    ...
    case kSectionDiagnose: {
        cell = [self tableView:tableView
            multiLineCellWithReuseIdentifier:@"diags"
                                        font:self.smallFont];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    ...
    case kSectionHtmlError: {
        cell = [self tableView:tableView
            multiLineCellWithReuseIdentifier:@"error"
                                        font:self.smallFont];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (NSString *)tableView:(UITableView *)tableView
    ...
    if ([self sectionType:section] == kSectionHtmlError) {
        return NSLocalizedString(@"There was a network problem:",
                                 @"section title");
    }
