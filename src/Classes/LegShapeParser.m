...
- (instancetype)init {
    if ((self = [super init])) {
        self.replaceQueryBlock = ^NSString *_Nonnull(
            LegShapeParser *_Nonnull xml, NSString *_Nonnull query) {
            NSMutableString *mutableQuery = query.mutableCopy;

            // The "URL" initially looks like this - we need to remove the
            // transweb part and add the trimet part
            // /transweb/ws/V1/BlockGeoWS/appID/xxxxx/bksTsIDeTeID/3305,X,11:21
            // AM,15,11:58 AM,7751

            [mutableQuery
                deleteCharactersInRange:NSMakeRange(
                                            0, 9)]; // /transweb is 9 characters
...
- (void)fetchCoords {
    @autoreleasepool {
        ...
        if (self.rawData) {
            ...
            NSDictionary *json =
                [NSJSONSerialization JSONObjectWithData:self.rawData
                                                options:0
                                                  error:&error];

            DEBUG_LOG_description(json);
            ...
            if (error) {
                ERROR_LOG(@"Error parsing JSON: %@\n%@",
                          ...
                          [[NSString alloc] initWithData:self.rawData
...
#define EXPECTED_CLASS(X, C) ((X) && [(X) isKindOfClass:[C class]])
            ...
            if (EXPECTED_CLASS(points, NSArray)) {
                ...
                for (NSDictionary *xy in points) {
                    ...
                    if (EXPECTED_CLASS(x, NSNumber) &&
                        EXPECTED_CLASS(y, NSNumber)) {
                        ...
                        //
                        // The coordinates are in the Oregon State Plane North
                        // (OSPN) projection. Frank Purcell has provided the
                        // math to convert this to lat and lng in
                        // http://groups.google.com/group/transit-developers-pdx
                        //
                        coord.longitude =
                            +((((atan(((xCoord * 0.3048) - 2500000) /
                                      (6350713.93 -
                                       ((yCoord * 0.3048) - 166910.7663)))) *
                                180) /
                               (3.14159265359 * 0.709186016884)) -
                              120.5);
                        coord.latitude =
                            (45.1687259619 +
                             ((((yCoord * 0.3048) - 166910.7663) -
                               (((xCoord * 0.3048) - 2500000) *
                                tan((atan(((xCoord * 0.3048) - 2500000) /
                                          (6350713.93 - ((yCoord * 0.3048) -
                                                         166910.7663)))) /
                                    2))) *
                              (0.000008999007999 +
                               (((yCoord * 0.3048) - 166910.7663) -
