...
#import "DepartureSortTableViewController.h"
...
enum { kTripRowFrom = 0, kTripRowTo, kTripRows };
...
@property(nonatomic, strong)
    NSMutableArray<id<DepartureTimesDataProvider>> *visibleDepartures;
@property(nonatomic, strong) NSMutableArray<XMLDepartures *> *xmlDepartures;
@property(nonatomic, strong)
    NSMutableDictionary<NSNumber *, Detour *> *allDetours;
@property(nonatomic, strong)
    NSMutableDictionary<NSString *, Route *> *allRoutes;
...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        _xmlDepartures = [NSMutableArray array];
        _visibleDepartures = [NSMutableArray array];
        _allDetours = [NSMutableDictionary dictionary];
        _allRoutes = [NSMutableDictionary dictionary];
        self.refreshFlags = kRefreshAll;

        DEBUG_LOG_long(depthCount);
        depthCount++;
        _updatedWatch = NO;
    }
...
- (bool)validStopDataProvider:(id<DepartureTimesDataProvider>)dd {
    return (dd.depGetSectionHeader != nil && dd.depDetour == nil &&
            (dd.depGetSafeItemCount == 0 ||
             (dd.depGetSafeItemCount > 0 &&
              [dd depGetDeparture:0].errorMessage == nil)));
}
...
- (NSMutableArray<Detour *> *)sortDetours:(NSMutableSet *)detoursNoLongerFound {
    ...
    [self.allDetours enumerateKeysAndObjectsUsingBlock:^void(
                         NSNumber *detourId, Detour *detour, BOOL *stop) {
      [detour.routes
          sortUsingComparator:^NSComparisonResult(Route *r1, Route *r2) {
...
- (void)sortByStop {
    NSMutableArray<id<DepartureTimesDataProvider>> *uiArray =
        [NSMutableArray array];
    NSMutableSet *detoursNoLongerFound =
        Settings.hiddenSystemWideDetours.mutableCopy;

    NSMutableArray<Detour *> *sortedDetours =
        [self sortDetours:detoursNoLongerFound];
        ...
        @autoreleasepool {
            ...
            if (deps.gotData) {
                for (bus = 0; bus < deps.count; bus++) {
                    ...
                    for (search = 0; search < self.visibleDepartures.count;
                         search++) {
                        busRoute = self.visibleDepartures[search];
                        firstItemForBus = [busRoute depGetDeparture:0];

                        if (itemToInsert.block != nil &&
                            [firstItemForBus.block
                                isEqualToString:itemToInsert.block]) {
                            for (insert = 0;
                                 insert < busRoute.departureItems.count;
                                 insert++) {
                                existingItem =
                                    [busRoute depGetDeparture:insert];

                                // existingItem is later in time than
                                // itemToInsert
                                if ([existingItem.departureTime
                                        compare:itemToInsert.departureTime] ==
                                    NSOrderedDescending) {
                                    [busRoute.departureItems
                                        insertObject:itemToInsert
                                             atIndex:insert];
                                    found = YES;
                                    break;
                                }
                    ...
                    if (!found) {
                        DepartureTimesByBus *newBus =
                            [[DepartureTimesByBus alloc] init];
...
- (void)cacheWarningRefresh:(bool)refresh {
    ...
    DEBUG_LOG_CGRect(self.tableView.frame);
}
...
- (SectionRows *)calcSubsections:(NSInteger)section {
    ...
    if (self.sectionExpanded == nil ||
        self.sectionExpanded.count != self.visibleDepartures.count) {
    ...
    if (sr[0].integerValue == kSectionRowInit) {
        ...
        // kSectionProximity
        if (dd.depLocation != nil && expanded &&
            [AlarmTaskList proximitySupported]) {
            next++;
        }
        ...
        // kSectionNearby
        if (dd.depLocation != nil && depthCount < kMaxDepth && expanded &&
        ...
        if (dd.depLocation != nil && depthCount < kMaxDepth && expanded &&
            multipleStops && [DepartureTimesViewController canGoDeeper]) {
            next++;
        }
        ...
        // kSectionVehicle
        if (dd.depLocation != nil && expanded && (Settings.vehicleLocations) &&
            [DepartureTimesViewController canGoDeeper]) {
            next++;
        }
        ...
        // kSectionNoDeeper
        if (expanded && ![DepartureTimesViewController canGoDeeper]) {
            next++;
        }
        ...
        // kSectionStation
        if (expanded && dd.depStopId != nil &&
            [StationData railstationFromStopId:dd.depStopId] != nil &&
            [DepartureTimesViewController canGoDeeper]) {
            next++;
        }
...
- (NSIndexPath *)subsection:(NSIndexPath *)indexPath;
{
    ...
    //    printf("Old %d %d new %d
    //    %d\n",(int)indexPath.section,(int)indexPath.row,
    //    (int)newIndexPath.section, (int)newIndexPath.row);
...
- (void)subTaskFetchTimesForVehicleStops:(NSString *)block
    ...
    @autoreleasepool {
        NSArray *batches =
            [XMLMultipleDepartures batchesFromEnumerator:self.vehicleStops
                                          selToGetStopId:@selector(stopId)
                                                     max:INT_MAX];

        while (batch < batches.count && items < maxToShow &&
               pos < self.vehicleStops.count && !done) {
            XMLMultipleDepartures *multiple =
                [XMLMultipleDepartures xmlWithOneTimeDelegate:taskState
                                                sharedDetours:self.allDetours
                                                 sharedRoutes:self.allRoutes];
                ...
                for (XMLDepartures *deps in multiple) {
                    if (items < maxToShow) {
                        if (deps.gotData && deps.count > 0 &&
                            !(deps.items.firstObject.status !=
                                  ArrivalStatusEstimated &&
                              deps.items.firstObject.minsToArrival > 59)) {
...
- (StopDistance *)subTaskFetchOtherDirectionForDeparture:(Departure *)dep
                                                   state:
                                                       (TaskState *)taskState {
    ...
    if (info && info->opposite != kNoDir && info->opposite != kDir1) {
        oppositeRoute =
            [NSString stringWithFormat:@"%ld", (long)info->opposite];
        ...
        [taskState taskSubtext:NSLocalizedString(@"checking direction",
                                                 @"progress message")];
        [xmlRoutes getDirections:oppositeRoute
        ...
        if (xmlRoutes.count > 0) {
            ...
            } else if (route.directions.count == 1 &&
                       [TriMetInfo isSingleLoopRoute:route.routeId]) {
    ...
    if (oppositeDirection) {
        ...
        [taskState taskSubtext:NSLocalizedString(@"finding stop",
                                                 @"progress message")];
        ...
        for (NSString *foundRoute in routes) {
            ...
            [parallelBlocks startBlock:^{
              [stops getStopsForRoute:foundRoute
            ...
            if (stops.count > 0) {
                for (Stop *stop in stops.items) {
                    CLLocationDistance dist =
                        [dep.stopLocation distanceFromLocation:stop.location];
...
- (NSMutableArray<StopDistance *> *)
    subTaskFindOppositeForOneRoute:(NSString *)bookmark
    ...
    StopDistance *stop =
        [self subTaskFetchOtherDirectionForDeparture:findOppositeToDep
                                               state:taskState];
...
- (NSMutableArray<StopDistance *> *)
    subTaskfindOppositeForManyRoutes:(NSString *)bookmark
    ...
    for (int i = 0; i < xmlFindOppositeToAll.count; i++) {
        ...
        for (Departure *d in uniqueRoutes) {
            if ([d.route isEqualToString:dep.route] &&
                [d.dir isEqualToString:dep.dir]) {
                found = d;
                break;
            }
        }
    ...
    for (Departure *d in uniqueRoutes) {
        ...
        [parallelBlocks startBlock:^{
          StopDistance *foundStop =
              [self subTaskFetchOtherDirectionForDeparture:d state:taskState];
              ...
              if (foundStop) {
                  for (int i = 0; i < oppositeStops.count; i++) {
                      ...
                      if ([stop.stopId isEqualToString:foundStop.stopId]) {
                          foundStop = nil;
                          break;
                      } else if (foundStop.distanceMeters <
                                 stop.distanceMeters) {
    ...
    if (!taskState.taskCancelled) {
        *stopIds =
            [NSString commaSeparatedStringFromEnumerator:oppositeStops
...
- (void)subTaskFetchMultipleStopsWithBlock:(NSString *)block
    ...
    for (int batch = 0; batch < batches; batch++) {
        ...
        NSArray<NSString *> *batchStopIdArray =
            [stopIdArray subarrayWithRange:NSMakeRange(start, batchSize)];

        [xmls addObject:multiple];

        [parallelBlocks startBlock:^{
          [multiple getDeparturesForStopIds:
                        [NSString commaSeparatedStringFromStringEnumerator:
                                      batchStopIdArray]];
          [taskState incrementItemsDoneAndDisplay];
          XML_DEBUG_RAW_DATA(multiple);
        }];
...
- (void)subTaskfetchSingleStopWithBlock:(NSString *)block
    ...
    XMLDepartures *deps = [XMLDepartures xmlWithOneTimeDelegate:taskState
                                                  sharedDetours:self.allDetours
                                                   sharedRoutes:self.allRoutes];
    ...
    if (names == nil) {
        [taskState
            taskSubtext:[NSString stringWithFormat:
                                      NSLocalizedString(
                                          @"Stop ID %@",
...
- (void)fetchTimesForStopId:(NSString *)stopId
    ...
    [taskController taskRunAsync:^(TaskState *taskState) {
      ...
      if (findOppositeToDep) {
          ...
          oppositeStops =
              [self subTaskfindOppositeForManyRoutes:bookmark
                                     findOppositeAll:xmlFindOppositeToAll
                                             stopIds:&foundStopIds
                                           taskState:taskState];
      } else {
          [taskState
              startAtomicTask:(bookmark != nil ? bookmark : kGettingArrivals)];
      }
      ...
      if (foundStopIds != nil && foundStopIds.length != 0) {
          NSArray<NSString *> *stopIdArray =
              foundStopIds.mutableArrayFromCommaSeparatedString;
          ...
          if (self.xmlDepartures.count > 0) {
              if (block != nil) {
                  self.title =
                      NSLocalizedString(@"Track Trip", @"screen title");
          ...
          [taskState
              taskSetErrorMsg:NSLocalizedString(
                                  @"Could not find a stop going the other way.",
                                  @"error message")];
      }
...
- (void)subTaskFetchMultipleStopsAgain:(TaskState *)taskState {
    ...
    for (int batch = 0; batch < batches; batch++) {
        ...
        for (i = start;
             i < self.xmlDepartures.count && !taskState.taskCancelled &&
             intbatch < kMultipleDepsMaxStops;
             i++) {
...
- (void)fetchTimesForVehicleAsync:(id<TaskController>)taskController
    ...
    [self fetchTimesForVehicleAsync:taskController
                              route:nil
                          direction:nil
                         nextStopId:nil
                              block:nil
...
- (void)fetchTimesForVehicleAsync:(id<TaskController>)taskController
    ...
    [self fetchTimesForVehicleAsync:taskController
                         ...
                         nextStopId:stopId
                              block:block
                    targetDeparture:targetDep
                          vehicleId:nil];
}

- (Vehicle *)subTaskFetchVehicleInfo:(TaskState *)taskState
                           vehicleId:(NSString *)vehicleId {
    taskState.total = 3;
    ...
    [locator findNearestVehicles:nil
                       direction:nil
                          blocks:nil
                        vehicles:[NSSet setWithObject:vehicleId]
                           since:nil];
    [taskState incrementItemsDoneAndDisplay];
...
- (void)subTaskFetchStopsAfterDeparture:(XMLStops *)stops
                              targetDep:(Departure *)targetDep
                              taskState:(TaskState *)taskState {
    ...
    for (DepartureTrip *trip in targetDep.trips) {
        ...
        if (stops.items.count > 0 && tripStops.items.count > 0 &&
            [stops.items.lastObject.stopId
                isEqualToString:tripStops.items.firstObject.stopId]) {
...
- (void)fetchTimesForVehicleAsync:(id<TaskController>)taskController
    ...
    [taskController taskRunAsync:^(TaskState *taskState) {
      ...
      if (vehicleId != nil) {
          Vehicle *vehicle = [self subTaskFetchVehicleInfo:taskState
                                                 vehicleId:vehicleId];
      ...
      if (!localBlock && targetDep == nil) {
          [taskState taskSetErrorMsg:
                         NSLocalizedString(
                             @"Vehicle not found, it may not be currently in "
                             @"service.  Note, Streetcar is not supported.",
                             @"error text")];
          [taskState taskCancel];
          return self;
      } else {
          ...
          if ((targetDep == nil || targetDep.trips == nil ||
               targetDep.trips.count == 0) &&
              localStopId != nil) {
              // Get stops in route after the current stop
              [stops
                  getStopsAfterStopId:localStopId
                                route:localRoute
              ...
              [self subTaskFetchStopsAfterDeparture:stops
                                          targetDep:targetDep
                                          taskState:taskState];
          }
          ...
          if (stops.gotData || stops.items.count > 0) {
              ...
              [self subTaskFetchTimesForVehicleStops:localBlock
                                           taskState:taskState];
          } else {
              [taskState taskCancel];
              [taskState
                  taskSetErrorMsg:
                      NSLocalizedString(
                          @"Could not find any departures for that vehicle.",
                          @"error message")];
          }
...
- (void)subTaskFetchArivalsForLocator:(XMLLocateStops *)locator
                            taskState:(TaskState *)taskState {
    ...
    NSArray *batches =
        [XMLMultipleDepartures batchesFromEnumerator:locator
                                      selToGetStopId:@selector(stopId)
                                                 max:locator.maxToFind];
    ...
    for (int batch = 0; batch < batches.count && !taskState.taskCancelled;
         batch++) {
        XMLMultipleDepartures *multiple =
            [XMLMultipleDepartures xmlWithOneTimeDelegate:taskState
                                            sharedDetours:self.allDetours
...
- (void)fetchTimesForNearestStopsAsync:(id<TaskController>)taskController
    ...
    [taskController taskRunAsync:^(TaskState *taskState) {
      ...
      if (location == nil) {
          [taskState taskStartWithTotal:kMultipleDepsBatches(max) + 2
                                  title:kGettingArrivals];

          location = [TaskLocator locateWithAccuracy:min taskState:taskState];

      } else {
          [taskState taskStartWithTotal:kMultipleDepsBatches(max) + 1
                                  title:kGettingArrivals];
      }

      if (!taskState.taskCancelled && location != nil) {
          [taskState taskSubtext:NSLocalizedString(@"getting locations",
                                                   @"progress message")];

          XMLLocateStops *locator =
              [XMLLocateStops xmlWithOneTimeDelegate:taskState];
          ...
          DEBUG_LOG(@"getting locatations");
          [locator findNearestStops];

          [taskState incrementItemsDoneAndDisplay];

          DEBUG_LOG(@"maybe show error");
          if (![locator displayErrorIfNoneFound:taskState]) {
              DEBUG_LOG(@"getting stops");
...
- (void)fetchTimesForNearestStopsAsync:(id<TaskController>)taskController
    ...
    [taskController taskRunAsync:^(TaskState *taskState) {
      ...
      int i = 0;
      NSArray *batches =
          [XMLMultipleDepartures batchesFromEnumerator:stops
                                        selToGetStopId:@selector(stopId)
                                                   max:INT_MAX];
...
- (void)fetchTimesViaQrCodeRedirectAsync:(id<TaskController>)taskController
    ...
    [taskController taskRunAsync:^(TaskState *taskState) {
      ...
      [taskState taskSubtext:NSLocalizedString(@"getting stop ID",
                                               @"progress message")];
      ...
      if (!taskState.taskCancelled && stopId) {
          ...
          [taskState
              taskSetErrorMsg:NSLocalizedString(
                                  @"That QR code is for the Portland Streetcar "
                                  @"website - there should be another QR code "
                                  @"close by that has the stop ID.",
                                  @"error message")];
      } else {
          [taskState taskCancel];
          [taskState
              taskSetErrorMsg:NSLocalizedString(
                                  @"The QR code is not for a TriMet stop.",
                                  @"error message")];
      }
...
- (UITableViewCell *)distanceCellWithReuseIdentifier:(NSString *)identifier {
    ...
    UITableViewCell *cell = [self tableView:self.tableView
                    cellWithReuseIdentifier:identifier];
...
#define LABEL_HEIGHT 26.0
    ...
    if ([cell viewWithTag:DISTANCE_TAG] == nil) {
        /*
         Create labels for the text fields; set the highlight color so that when
         the cell is selected it changes appropriately.
         */
        UILabel *label;

        rect = CGRectMake(LEFT_COLUMN_OFFSET,
                          (kDepartureCellHeight / 2.0 - LABEL_HEIGHT) / 2.0,
                          LEFT_COLUMN_WIDTH, LABEL_HEIGHT);
        label = [[UILabel alloc] initWithFrame:rect];
        label.tag = DISTANCE_TAG;
        label.font =
            [UIFont boldMonospacedDigitSystemFontOfSize:MAIN_FONT_SIZE];
...
- (void)refreshAction:(id)sender {
    if (!self.backgroundTask.running) {
        ...
        if (self.tableView.hidden || self.backgroundTask.progressModal != nil) {
            return;
        }
...
- (void)subTaskFetchStreetcarLocations:(TaskState *)taskState {
    NSSet<NSString *> *streetcarRoutes = [XMLStreetcarLocations
        getStreetcarRoutesInXMLDeparturesArray:self.xmlDepartures];
    ...
    [XMLStreetcarLocations
        insertLocationsIntoXmlDeparturesArray:self.xmlDepartures
                                    forRoutes:streetcarRoutes];
}

- (void)showMapNow:(id)sender {
    MapViewControllerWithRoutes *mapPage =
        [MapViewControllerWithRoutes viewController];
    ...
    if (_blockFilter) {
        mapPage.title =
            NSLocalizedString(@"Stops & Departures", @"screen title");

        needStreetcarLocations =
            [self needtoFetchStreetcarLocations:self.xmlDepartures];

        if (needStreetcarLocations) {
            NSSet<NSString *> *streetcarRoutes = [XMLStreetcarLocations
                getStreetcarRoutesInXMLDeparturesArray:self.xmlDepartures];
            additonalTasks =
                1 + streetcarRoutes.count +
                ((int)self.xmlDepartures.count * (int)streetcarRoutes.count);
        }
    } else {
    ...
    [mapPage
        fetchRoutesAsync:self.backgroundTask
                  routes:routes
                          ...
                          if (deps.loc != nil) {
                              ...
                              if (self->_blockFilter) {
                                  for (j = 0; j < deps.count; j++) {
                                      ...
                                      if (departure.hasBlock &&
                                          ![blocks
                                              containsObject:departure.block] &&
                                          departure.blockPosition != nil) {
                                      ...
                                      if (departure.hasBlock &&
                                          ...
                                          if (!found) {
                                              [routes
                                                  addObject:departure.route];
                                              [directions
                                                  addObject:departure.dir];
                                          }
                                      }
                                  }
                              }
...
- (void)sortButton:(id)sender {
    DepartureSortTableViewController *options =
        [DepartureSortTableViewController viewController];
...
- (void)bookmarkButton:(UIBarButtonItem *)sender {
    ...
    if (self.xmlDepartures.count == 1) {
        ...
        XMLDepartures *deps = self.xmlDepartures.firstObject;
        [stopId appendFormat:@"%@", deps.stopId];
        [desc appendFormat:NSLocalizedString(@"Stop IDs: %@",
                                             @"A list of TriMet stop IDs"),
                           deps.stopId];
    ...
    @synchronized(_userState) {
        for (i = 0; _userState.faves != nil && i < _userState.faves.count;
            ...
            NSString *faveLoc = (NSString *)bm.valLocation;

            if (bm != nil && faveLoc != nil &&
                [faveLoc isEqualToString:stopId]) {
                bookmarkItem = i;
                desc = bm.valChosenName.mutableCopy;
                break;
            }
        }
    }
    ...
    if (bookmarkItem == kNoBookmark) {
        UIAlertController *alert = [UIAlertController
            alertControllerWithTitle:NSLocalizedString(@"Bookmark",
                                                       @"action list title")
                             message:nil
        ...
        [alert
            addAction:
                [UIAlertAction
                    actionWithTitle:NSLocalizedString(@"Edit this bookmark",
                                                      @"button text")
                              style:UIAlertActionStyleDefault
                            handler:^(UIAlertAction *action) {
                              EditBookMarkViewController *edit =
                              ...
                              @synchronized(self->_userState) {
                                  [edit editBookMark:self->_userState
                                                         .faves[bookmarkItem]
                                                item:bookmarkItem];
                              }
                              // Push the detail view controller
...
- (CGFloat)tableView:(UITableView *)tableView
    ...
    CGFloat result;
    id<DepartureTimesDataProvider> dd =
        self.visibleDepartures[indexPath.section];
    NSIndexPath *newIndexPath = [self subsection:indexPath];

    switch (newIndexPath.section) {
    case kSectionTimes:

        if (dd.depGetSafeItemCount == 0 && newIndexPath.row == 0 &&
            !_blockFilter) {
...
- (NSInteger)tableView:(UITableView *)tableView
    ...
    // (long)sr->row[kSectionsPerStop-1],
...
- (UITableViewCell *)actionCell:(UITableView *)tableView
    ...
    UITableViewCell *cell = [self tableView:tableView
                    cellWithReuseIdentifier:kActionCellId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    switch (newIndexPath.section) {
    case kSectionSystemAlert: {
        cell = [self detourCell:dd.depDetour indexPath:indexPath];
        break;
    }
    ...
    case kSectionDistance: {
        ...
        if (twoLines) {
            ...
            NSString *distance = [NSString
                stringWithFormat:NSLocalizedString(@"Distance %@",
                                                   @"stop distance"),
                                 [FormatDistance
                                     formatMetres:dd.depDistance
                                                      .distanceMeters]];
            ((UILabel *)[cell.contentView viewWithTag:DISTANCE_TAG]).text =
                distance;
            UILabel *accuracy =
                (UILabel *)[cell.contentView viewWithTag:ACCURACY_TAG];
            accuracy.text = [NSString
                stringWithFormat:NSLocalizedString(
                                     @"Accuracy +/- %@",
                                     @"accuracy of location services"),
                                 [FormatDistance
                                     formatMetres:dd.depDistance.accuracy]];
            cell.accessibilityLabel =
                [NSString stringWithFormat:@"%@, %@", distance, accuracy.text];
        } else {
            cell = [self tableView:tableView
                cellWithReuseIdentifier:kDistanceCellId];
    ...
    case kSectionTimes: {
        ...
        if (deps == 0 && i == 0) {
            cell = [self tableView:tableView
                cellWithReuseIdentifier:kStatusCellId];

            if (_blockFilter) {
                cell.textLabel.text = NSLocalizedString(
                    @"No departure data for that particular trip.",
                    @"error message");
                ...
                cell.textLabel.text = [NSString
                    stringWithFormat:
                        NSLocalizedString(
                            @"Stop ID %@ - no departures",
                            @"No departures for a specific TriMet stop ID"),
                        dd.depStopId];
                cell.textLabel.font = self.basicFont;
            }
            ...
            // Crash logs show this is sometimes of bounds, so be defensive.

            if (departure) {
                DepartureCell *dcell =
                    [DepartureCell tableView:tableView
                        cellWithReuseIdentifier:MakeCellId(kSectionTimes)
                                 tallRouteLabel:NO];

                [dd depPopulateCell:departure
                               cell:dcell
                           decorate:YES
                               wide:LARGE_SCREEN];
                ...
                // A dummy cell
                cell = [super tableView:tableView
    ...
    case kSectionDetours: {
        ...
        if (det != nil) {
            DetourTableViewCell *dcell = [self detourCell:det
                                                indexPath:indexPath];
    ...
    case kSectionStatic:
        ...
        if (newIndexPath.row == 0) {
            ...
            if (dd.depNetworkError) {
                ...
                button.frame = CGRectMake(0, 0, ACCESSORY_BUTTON_SIZE,
                                          ACCESSORY_BUTTON_SIZE);

                NSString *icon =
                    self.sectionExpanded[indexPath.section].boolValue
                        ? kSFIconChevronUp
                        : kSFIconChevronDown;

                [button setImage:[UIImage systemImageNamed:icon]
            ...
            for (int i = 0;
                 i < dd.depGetSafeItemCount && streetcarDisclaimer == nil;
                 i++) {
    ...
    case kSectionNearby:
        cell =
            [self actionCell:tableView
                 systemImage:kSFIconLocateNearby
    ...
    case kSectionSiri: {
        cell = [self actionCell:tableView
                     imageNamed:kIconSiri
                           text:NSLocalizedString(@"Add this stop to Siri",
                                                  @"button text")
                      accessory:UITableViewCellAccessoryDisclosureIndicator];
    ...
    case kSectionOneStop:
        cell = [self actionCell:tableView
                    systemImage:kSFIconArrivals
                           text:NSLocalizedString(@"Show only this stop",
                                                  @"button text")
                      accessory:UITableViewCellAccessoryDisclosureIndicator];
    ...
    case kSectionStation:
        cell = [self actionCell:tableView
                     imageNamed:kIconRailStations
                           text:NSLocalizedString(@"Rail station details",
                                                  @"button text")
                      accessory:UITableViewCellAccessoryDisclosureIndicator];
        break;
    ...
    case kSectionFilter:
        cell = [self actionCell:tableView
                    systemImage:kSFIconLocateMe
                           text:self.savedBlock
                                    ? NSLocalizedString(@"Show one departure",
                                                        @"button text")
                                    : NSLocalizedString(@"Show all departures",
                                                        @"button text")
                      accessory:UITableViewCellAccessoryDisclosureIndicator];
        break;
    ...
    case kSectionAccuracy:
        cell = [self actionCell:tableView
                     imageNamed:kIconTriMetLink
                           text:NSLocalizedString(@"Check TriMet website",
                                                  @"button text")
                      accessory:UITableViewCellAccessoryDisclosureIndicator];
        break;
...
- (void)tableView:(UITableView *)table
    siriButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    ...
    intent.suggestedInvocationPhrase =
        [NSString stringWithFormat:@"TriMet departures at %@", deps.locDesc];
...
- (void)tableView:(UITableView *)tableView
    ...
    switch (newIndexPath.section) {
    case kSectionTimes: {
        if (dd.depGetSafeItemCount != 0 &&
            newIndexPath.row < dd.depGetSafeItemCount) {
            Departure *departure = [dd depGetDeparture:newIndexPath.row];

            // if (departure.hasBlock || departure.detour)
            if (departure.errorMessage == nil) {
                DepartureDetailViewController *departureDetailView =
                    [DepartureDetailViewController viewController];
                departureDetailView.stopIdStringCallback =
                    self.stopIdStringCallback;
                departureDetailView.delegate = self;

                departureDetailView.navigationItem.prompt =
                    self.navigationItem.prompt;

                if (depthCount < kMaxDepth &&
                    self.visibleDepartures.count > 1) {
                    departureDetailView.stops = self.stopIds;
                }

                departureDetailView.allowBrowseForDestination =
                    ((!_blockFilter) || Settings.vehicleLocations) &&
                    depthCount < kMaxDepth;

                [departureDetailView fetchDepartureAsync:self.backgroundTask
                                                     dep:departure
                                           allDepartures:self.xmlDepartures
            ...
            [taskList
                userAlertForProximity:self
                           ...
                           completion:^(bool cancelled, bool accurate) {
                             if (!cancelled) {
                                 [taskList
                                     addTaskForStopIdProximity:dd.depStopId
                                                           loc:dd.depLocation
                                                          desc:dd.depLocDesc
                                                      accurate:accurate];
                                 [self reloadData];
    ...
    case kSectionNearby: {
        FindByLocationViewController *find =
            [[FindByLocationViewController alloc]
                initWithLocation:dd.depLocation
                     description:dd.depLocDesc];
    ...
    case kSectionSiri:
        [self tableView:self.tableView
            siriButtonTappedForRowWithIndexPath:indexPath];
        break;
    ...
    case kSectionStation: {
        {
            RailStation *station =
                [StationData railstationFromStopId:dd.depStopId];
    ...
    case kSectionOneStop: {
        ...
        departureViewController.stopIdStringCallback =
            self.stopIdStringCallback;
    ...
    case kSectionStatic: {
        if (dd.depNetworkError) {
            ...
            [self.tableView deselectRowAtIndexPath:indexPath animated:YES];
            self.sectionExpanded[sect] =
                self.sectionExpanded[sect].boolValue ? @NO : @YES;
            ...
            for (int i = 0; i < kSectionsPerStop; i++) {
                ...
                if (newRows[i + 1].integerValue - newRows[i].integerValue !=
                    oldRows[i + 1].integerValue - oldRows[i].integerValue) {
                    for (row = additionalRows[i].intValue;
                         row < additionalRows[i + 1].intValue; row++) {
                        [changingRows
                            addObject:[NSIndexPath indexPathForRow:row
                                                         inSection:sect]];
                    }
                }
            }

            UITableViewCell *staticCell =
            ...
            if (self.sectionExpanded[sect].boolValue) {
                [self.tableView
                    insertRowsAtIndexPaths:changingRows
                          withRowAnimation:UITableViewRowAnimationRight];
            } else {
                [self.tableView
                    deleteRowsAtIndexPaths:changingRows
                          withRowAnimation:UITableViewRowAnimationRight];
            }
...
- (void)updateToolbarItems:(NSMutableArray *)toolbarItems {
    ...
    if (((!(_blockFilter || self.xmlDepartures.count == 1)) || _allowSort) &&
        (Settings.groupByArrivalsIcon)) {
        UIBarButtonItem *sort =
            [UIBarButtonItem withSystemImage:kSFIconSort
                                       style:style
                                      target:self
                                      action:@selector(sortButton:)];

        sort.accessibilityLabel =
            NSLocalizedString(@"Group Departures", @"Accessibility text");
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    for (XMLDepartures *deps in self.xmlDepartures) {
        WorkerTask((^{
          ...
          intent.suggestedInvocationPhrase = [NSString
              stringWithFormat:@"TriMet departures at %@", deps.locDesc];
          intent.stops = deps.stopId;
          ...
          [interaction
              donateInteractionWithCompletion:^(NSError *_Nullable error) {
                LOG_NSError(error);
              }];
        }));
    }
    ...
    if (userActivityClass != nil) {
        NSMutableString *stopIds =
            [NSString commaSeparatedStringFromEnumerator:self.xmlDepartures
                                                selector:@selector(stopId)];
        ...
        if (self.xmlDepartures.count >= 1) {
            ...
            self.userActivity.webpageURL = [NSURL
                URLWithString:[WebViewController namedURL:@"TriMet Arrivals"
                                                    param:deps.stopId]];

            if (self.xmlDepartures.count == 1 && deps.locDesc != nil) {
                info.valChosenName = deps.locDesc;
            } else if (self.bookmarkDesc != nil) {
                info.valChosenName = self.bookmarkDesc;
            } else {
                info.valChosenName = @"unknown stops";
            }

            if (self.bookmarkDesc != nil) {
                self.userActivity.title = [NSString
                    stringWithFormat:kUserFavesDescription, self.bookmarkDesc];
            } else {
                self.userActivity.title = [NSString
                    stringWithFormat:
                        @"Launch PDX Bus & show departures for stops %@",
                        stopIds];
            }
