...
@interface TripPlannerDateViewController ()

@property(nonatomic, strong) UIDatePicker *datePickerView;

@end

@implementation TripPlannerDateViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    case kSectionDateButtons: {
        cell = [self tableView:tableView
            cellWithReuseIdentifier:MakeCellId(kSectionDateButtons)];
        switch (row) {
        case kSectionDateDepartNow:
        default:
            cell.textLabel.text =
                NSLocalizedString(@"Depart now", @"button text");
            break;

        case kSectionDateDeparture:
            cell.textLabel.text = NSLocalizedString(
                @"Depart after the time below", @"button text");
            break;

        case kSectionDateArrival:
            cell.textLabel.text =
                NSLocalizedString(@"Arrive by the time below", @"button text");
            break;
        }
        cell.textLabel.font = self.basicFont;
        cell.accessoryType = self.popBack
    ...
    case kSectionDatePicker: {
        cell = [tableView
            dequeueReusableCellWithIdentifier:MakeCellId(kSectionDatePicker)];
...
- (void)nextScreen:(UINavigationController *)controller
    ...
    if (self.tripQuery.userRequest.timeChoice == TripAskForTime) {
        ...
        self.tripQuery.userRequest.arrivalTime =
            (self.tripQuery.userRequest.timeChoice == TripArriveBeforeTime);
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[DatePickerCell nib]
         forCellReuseIdentifier:MakeCellId(kSectionDatePicker)];
