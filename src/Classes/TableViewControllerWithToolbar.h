...
#define kNoNetwork                                                             \
    NSLocalizedString(@"%@: touch here for info", @"Stop ID and message")
#define kNoNetworkErrorID                                                      \
    NSLocalizedString(@"(ID %@) %@: touch here for info",                      \
                      @"Stop ID and message")
#define kNoNetworkID                                                           \
    NSLocalizedString(@"(ID %@) No Network: touch here for info",              \
                      @"Stop ID and error message")
#define kNetworkMsg                                                            \
    NSLocalizedString(@"Network error: touch here for info",                   \
...
#define MakeMapRectWithPointAtCenter(X, Y, W, H)                               \
    MKMapRectMake((X) - (W) / 2, (Y) - (H) / 2, W, H)
...
@interface TableViewControllerWithToolbar<FilteredItemType>
    : ViewControllerBase <
          ...
          UISearchControllerDelegate, DeselectItemDelegate, MKMapViewDelegate,
          UISearchBarDelegate, INUIAddVoiceShortcutViewControllerDelegate> {
...
- (UITableViewCell *)getMapCell:(NSString *)id
               withUserLocation:(bool)userLocation
                     completion:(void (^)(MKMapView *map))completion;
