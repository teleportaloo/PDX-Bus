...
- (void)stopScanner {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // An open or a close brace will end the search for stop Ids in a sequence. An open brace is
        // unexpected so stopping is the safest.
        numbersOrBrace = [NSCharacterSet characterSetWithCharactersInString:@"0123456789)("];
        numbers = [NSCharacterSet characterSetWithCharactersInString:@"0123456789"];
        
        searchStrings = @[ @"(stop id",  @"( stop id", @"#h"];    // String is made lower case
    });
    ...
    for (NSString *prefix in searchStrings) {
        ...
        while (!scanner.isAtEnd) {
            ...
            if (!scanner.isAtEnd && scanner.scanLocation + prefix.length < scanString.length) {
                NSString *text = [scanString substringWithRange:NSMakeRange(scanner.scanLocation, prefix.length)];
                ...
                while (!scanner.isAtEnd) {
                    ...
                    // If we have a # we do not tolerate junk between the # and the number
                    ...
                    if (!scanner.isAtEnd) {
                        ...
                        unichar charAtIndex = [scanString characterAtIndex:scanner.scanLocation];
            ...
            {
                // There may be a malformed search string at the end.  This ensures no characters are lost even the junk.
                NSString *text = [scanString substringWithRange:NSMakeRange(scanner.scanLocation, scanString.length-scanner.scanLocation)];
                [result appendString:text];
                break;
            }
            else
                     ...
                     addEmoji:(bool)addEmoji
{
    ...
    if ([url containsString:@" "])
    {
        ...
        for (NSString *prefix in prefixes)
        {
            ...
            NSRange lastURL = [url rangeOfString:prefix options:NSCaseInsensitiveSearch | NSBackwardsSearch];
