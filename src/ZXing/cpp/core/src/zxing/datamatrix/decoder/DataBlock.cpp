 ...
 *  PDX Bus changes (c) 2012 A.R.Wallace
 *
...
std::vector<Ref<DataBlock> > DataBlock::getDataBlocks(ArrayRef<unsigned char> rawCodewords, Version *version) {
  ...
  int shorterBlocksTotalCodewords = result[0]!=0 ? (int)result[0]->codewords_.size() : 0;
  ...
  // Now add in error correction blocks
    int max = result[0]!=0 ? (int)result[0]->codewords_.size() : 0;
