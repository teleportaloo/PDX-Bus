...
- (void)testAbout {
    ...
    for (NSDictionary *dict in sut) {
        [self.linkChecker checkLink:dict.linkInfo.valLinkFull];
        [self.linkChecker checkLink:dict.linkInfo.valLinkMobile];
    }
...
- (void)testOtherLinks {
    NSDictionary<NSString *, NSDictionary *> *sut = [NSDictionary
    ...
    [sut enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
                                             NSDictionary *_Nonnull obj,
                                             BOOL *_Nonnull stop) {
      NSString *testString = obj[@"TestString"];
      NSString *testString2 = obj[@"TestString2"];

      NSString *full = obj.linkInfo.valLinkFull;
      NSString *mobile = obj.linkInfo.valLinkMobile;
