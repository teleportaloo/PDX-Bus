...
#import "../PDXBusCore/src/NSString+MoreMarkup.h"
...
- (void)test_XMLDetours_systemWide {
    ...
    [xmlFile tagWithAtrributes:@"alert "
                    attributes:@[
                        @[ @"begin", @"1495713600000" ], @[ @"end", @"" ],
                        @[ @"id", @"1" ],
                        ...
                        @[
                            @"desc",
                            @"A system-wide event with balloons is happening."
                        ]
                    ]];

    [xmlFile closeHeaderTag];

    sut.queryTransformer = xmlFile.queryBlock;
    ...
    XCTAssertEqualObjects(sut[0].detourDesc,
                          @"A system-wide event with balloons is happening.");
...
- (void)test_XMLDetours_secondUrl {
    ...
    [xmlFile tagWithAtrributes:@"alert "
                    attributes:@[
                        ...
                        @[
                            @"info_link_url",
                            @"https://trimet.org https://gobytram.org"
                        ],
                        @[ @"header_text", @"Thanksgiving Day Service" ],
                        @[ @"system_wide_flag", @"true" ],
                        @[
...
- (void)test_XMLDetours_secondUrlUpperCase {
    ...
    [xmlFile tagWithAtrributes:@"alert "
                    attributes:@[
                        ...
                        @[
                            @"info_link_url",
                            @"https://trimet.org HTTP://gobytram.org"
                        ],
                        @[ @"header_text", @"Thanksgiving Day Service" ],
                        @[ @"system_wide_flag", @"true" ],
                        @[
...
- (void)test_XMLDetours_badScan {
    ...
    [xmlFile
        startTagWithAttributes:@"alert"
                    attributes:@[
                        ...
                        @[
                            @"desc",
                            @"Hello (Stop IDs bad (Stop ID 12 is bad so is 16 and 17) # nope 101 #detour (Stop Id(stop id 18) (Stop Idx 1234567) #"
                        ]
                    ]];

    [xmlFile tagWithAtrributes:@"route"
                    attributes:@[
    ...
    XCTAssertEqualObjects(result.beginDate,
                          [NSDate dateWithTimeIntervalSince1970:1495713600]);
...
- (void)test_XMLDetours_routeAndStops {
    ...
    [xmlFile
        startTagWithAttributes:@"alert"
                    attributes:@[
                        ...
                        @[
                            @"desc",
                            @"No service to northbound stops on SW 158th Ave at Jenkins (Stop ID 8234), Jay (Stop ID 6839), Baseline (Stop ID 8232) and Greystone Ct (Stop ID 13202), due to construction. Use stops before or after at Merlo Rd/SW 158th MAX Stn (Stop ID 12899) or SW 158th &amp; Walker (Stop ID 6832). Test (Stop ID bad) (Stop ID "
                        ]
                    ]];
    ...
    [xmlFile
        tagWithAtrributes:@"location"
               attributes:@[
                   @[ @"id", @"7122" ], @[ @"desc", @"NW 21st &amp; Lovejoy" ],
                   @[ @"dir", @"Northbound" ], @[ @"lat", @"45.530113999998" ],
                   @[ @"lng", @"-122.69447499998" ],
                   @[ @"no_service_flag", @"false" ],
                   @[ @"passengerCode", @"E" ]
               ]];
    ...
    [xmlFile
        startTagWithAttributes:@"alert"
                    attributes:@[
                        ...
                        @[
                            @"desc",
                            @"#28 29 # 30 49 No service to northbound stops on SW 158th Ave at Jenkins (Stop Id (Stop ID 15( ( Stop ids 8234), Jay ( Stop id 6839,1,2,3,4,M), Baseline (Stop IDs 8232,5) and Greystone Ct (Stop ID 13202), due to construction. Use stops before or after at Merlo Rd/SW 158th MAX Stn (Stop ID 12899) or SW 158th &amp; Walker (Stop ID 6832). Test (Stop ID bad) (Stop ID "
                        ]
                    ]];
...
- (void)test_XMLDetours_realDetours {
    ...
    sut.queryTransformer =
        [XMLTestFile queryBlockWithFileForClass:@{@"XMLDetours" : @"detours"}];
    ...
    XCTAssertEqualObjects(
        result.detourDesc,
        @"The stop at SW Columbia & 1st (Stop ID 12795) is closed long-term "
        @"for construction. Use the temporary stop at SW Columbia & 2nd (Stop "
        @"ID 14053).");
    XCTAssertEqualObjects(result.detectStops.removeMarkUp, result.detourDesc);
