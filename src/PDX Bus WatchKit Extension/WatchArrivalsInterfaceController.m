...
- (void)loadTableWithDepartures:(XMLDepartures *)newDepartures detailedDeparture:(Departure *)newDetailDep {
    ...
    switch (deps.count) {
        ...
        case 1: {
            ...
            if (first.errorMessage) {
                ...
                if (deps.firstObject.sortedDetours.detourIds && deps.firstObject.sortedDetours.detourIds.count > 0) {
                    NSInteger i;
                    Departure *first = deps.firstObject;
                    ...
                    for (i = 0; i < first.sortedDetours.systemWideCount; i++) {
                        if (![hidden containsObject:first.sortedDetours.detourIds[i]]) {
                    ...
                    if (i < first.sortedDetours.detourIds.count) {
                        ...
                        [rowIndices addObject:@(first.sortedDetours.detourIds.count - i)];
    ...
    if (newDepartures.gotData) {
        ...
        NSDictionary *recent = [userData addToRecentsWithStopId:self.arrivalsContext.stopId description:longDesc];
...
- (void)extractDetours:(XMLDepartures *)departures {
    ...
    [departures.detourSorter.allDetours enumerateKeysAndObjectsUsingBlock: ^void (NSNumber *detourId, Detour *detour, BOOL *stop)
     {
        ...
        if (newDetailDep) {
            if (newDetailDep.needToFetchStreetcarLocation && !self.backgroundThread.cancelled) {
                ...
                if (newDetailDep.streetcarId == nil) {
                    ...
                    XMLStreetcarPredictions *streetcarArrivals = [XMLStreetcarPredictions xmlWithOneTimeDelegate:self];
                    
                    [streetcarArrivals getDeparturesForStopId:newDetailDep.stopId];
                ...
                XMLLocateVehicles *vehicles = [XMLLocateVehicles xmlWithOneTimeDelegate:self];
